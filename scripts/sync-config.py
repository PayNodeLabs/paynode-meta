...
def sync_file(path, new_content, check_mode=False):
    ...
    if not os.path.exists(os.path.dirname(path) if os.path.dirname(path) else "."):
...
def sync_config(check_mode=False):
    ...
    python_content = f'''# Generated by scripts/sync-config.py
...
'''
    ...
    js_content = f'''/** Generated by scripts/sync-config.py */
...
'''
    ...
    web_content = f'''/** Generated by scripts/sync-config.py */
export const PROTOCOL_TREASURY = "{config["protocol"]["treasury"]}";
export const PROTOCOL_FEE_BPS = {config["protocol"]["fee_bps"]};
export const MIN_PAYMENT_AMOUNT = {min_amount};
...
'''
    ...
    ai_skills_utils_path = "packages/paynode-ai-skills/paynode-402/scripts/utils.ts"
    if os.path.exists(ai_skills_utils_path):
        with open(ai_skills_utils_path, "r") as f:
            content = f.read()
    ...
    if check_mode:
        ...
        print("📝 Note: SKILL_VERSION in AI Skills is not managed by this script.")
        print("   Please ensure it is manually aligned in packages/paynode-ai-skills/paynode-402/scripts/utils.ts")
